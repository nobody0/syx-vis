...
function serializeToBinary() {
  const idBytes = new TextEncoder().encode(state.buildingId);
  const flat = state.room.flat();
  ...
  const placementsLen = 1 + state.placements.length * 5;
  ...
  // Placements
  buf[off++] = state.placements.length;
 ...
 */
function parseBinaryPlan(bytes) {
  let off = 0;
...
// Pre-compute stability rays using Bresenham from origin to Chebyshev-radius-4 perimeter
